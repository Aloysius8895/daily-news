...
4. 每条 summary 用中文写成 2 到 3 句，简洁、信息密度高，不要夸张，不要加猜测。
5. importance 字段写一句中文，解释为什么这条值得进入日报。
6. 如果原始数据里没有足够信息，不要编造；缺失字段可以留空字符串。
7. 最多输出 12 条新闻，优先保留国际影响大、政策变化大、金融市场影响大的事件。
